-- Correct two leads wrongly shown as Policy Done (already renewed elsewhere).
-- Singh Wadhera Wadhera — RJ45CP8460 · Jitendra Sindal
-- Prasad Chaturvedi Chaturvedi — 8076220230 · Manoj Sharma
-- Run in Supabase SQL Editor, then refresh Admin → Policy Done tab.
...
WITH targets AS (
  ...
  WHERE a.status IN ('policy_done', 'already_renewed_unknown')
    AND (
      upper(replace(coalesce(s.vehicle_registration_number, ''), ' ', '')) = 'RJ45CP8460'
      OR (
        regexp_replace(coalesce(s.contact_phones, ''), '\D', '', 'g') LIKE '%8076220230%'
        AND lower(coalesce(s.first_name, '') || ' ' || coalesce(s.last_name, '')) LIKE '%chaturvedi%'
      )
    )
)
...
WITH affected AS (
  ...
  FROM public.insurance_renewal_assignments a
  JOIN public.all_service_data s ON s.id = a.customer_id
  WHERE upper(replace(coalesce(s.vehicle_registration_number, ''), ' ', '')) = 'RJ45CP8460'
     OR (
       regexp_replace(coalesce(s.contact_phones, ''), '\D', '', 'g') LIKE '%8076220230%'
       AND lower(coalesce(s.first_name, '') || ' ' || coalesce(s.last_name, '')) LIKE '%chaturvedi%'
     )
...
-- Verify (expect status = renewed_elsewhere for both)
SELECT
