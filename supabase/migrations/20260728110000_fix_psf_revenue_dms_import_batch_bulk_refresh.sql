-- Fix DMS revenue import timeout: replace per-job-card refresh loop with one bulk UPDATE.
-- run_psf_revenue_dms_import_batch was calling refresh_job_card_closed_dms_revenue once per
-- distinct job card in each chunk (up to 500 sequential queries), exceeding statement_timeout.
...
BEGIN
  ...
  target AS (
    ...
    INNER JOIN keys k
      ON upper(btrim(jc.job_card_number)) = k.job_card_number
     AND jc.location = k.location
     AND jc.portal = k.portal
...
COMMENT ON FUNCTION public.refresh_job_card_closed_dms_revenue_batch(jsonb) IS
  'Bulk mirror latest psf_revenue_dms amounts into matching job_card_closed_data rows for a JSON array of job_card_number/location/portal keys.';
...
COMMENT ON FUNCTION public.run_psf_revenue_dms_import_batch(jsonb) IS
  'Bulk upsert psf_revenue_dms from JSON array (Import page). Disables per-row JCC DMS sync trigger during merge; bulk-refreshes affected job cards once at end.';
