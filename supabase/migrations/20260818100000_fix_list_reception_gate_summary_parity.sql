-- Unify list_reception_entries_page / list_reception_entries_page_for_admin gates with
-- get_service_advisor_summary_counts so summary KPIs and assigned list always match.
--
-- 20260817180000 over-corrected for vk1: v_is_floor_or_bodyshop_only blocked the advisor
-- JOIN for CRM/broad-scope users who also had floor_incharge module (service@techwheels.in:
-- summary 656, list 0). Floor path then ran without broad_scope guard and returned nothing.
--
-- Advisor path (match summary exactly):
--   NOT admin AND NOT user_needs_sa_summary_broad_scope() AND active user_employee_links
-- Floor path only when NOT broad_scope AND advisor path did not apply.
...
IS 'Paginated reception list. Advisor JOIN gate matches get_service_advisor_summary_counts exactly. '
   'Floor path only when NOT broad_scope and no employee-link advisor path; reception-dealer for unlinked receptionists.';
