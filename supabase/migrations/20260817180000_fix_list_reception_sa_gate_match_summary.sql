-- Fix assigned SA seeing summary counts but zero list rows (vk1.techwheels@gmail.com / VK1_3000840).
--
-- Regression from 20260817150000 / 20260817170000: list_reception_entries_page required
-- service_advisor module AND blocked the own-code JOIN when floor_incharge / bodyshop_floor
-- modules or FLOOR_INCHARGE business role were present.
--
-- get_service_advisor_summary_counts uses the simpler gate:
--   NOT admin AND NOT user_needs_sa_summary_broad_scope() → JOIN user_employee_links
--
-- Align list RPCs to that gate. Floor/bodyshop-only users (no SA module) still use floor path.
...
IS 'Paginated reception list. SA own-code gate matches get_service_advisor_summary_counts. '
   'Floor-only users (no SA module) use floor SA JOIN; reception-dealer for unlinked receptionists.';
