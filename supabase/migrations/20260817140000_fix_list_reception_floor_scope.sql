-- Restore bodyshop-floor / floor-incharge / reception-only lists after
-- 20260817120000 over-applied the assigned-SA JOIN fast path.
--
-- 20260817120000 used: NOT admin AND NOT user_needs_sa_summary_broad_scope()
-- (CRM/SM/GM only). That forced bodyshop_floor users onto
-- user_employee_links.employee_code = sa_employee_code. Floor staff are not
-- the assigned SA on accident cars, so Bodyshop Floor showed 0 vehicles.
-- Reception-only and floor_incharge callers hit the same empty JOIN.
--
-- Keep the SA+reception 57014 fix: assigned advisors (even with reception
-- module) still use the own-code JOIN. Shared-list roles fall through to
-- service_reception_entry_in_summary_scope / reception_dealer_only.
...
IS 'Paginated reception list bypassing authenticated-role RLS (57014). '
   'Advisor own-code JOIN is only for assigned SA (not bodyshop_floor / floor_incharge).';
...
IS 'Reg numbers + created_at since timestamp; advisor own-code JOIN excludes bodyshop_floor / floor_incharge.';
