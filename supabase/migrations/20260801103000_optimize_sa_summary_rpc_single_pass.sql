-- Optimize get_service_advisor_summary_counts: single-pass aggregation (fixes 57014 on Last Month).
--
-- Problems fixed:
-- 1) Multiple subquery counts re-scanned enriched/base CTEs (slow on 200+ rows).
-- 2) service_reception_entry_in_summary_scope called is_admin() per row.
-- 3) Advisor scope index: (sa_employee_code, created_at DESC).
--
-- Apply after 20260731143000:
--   node scripts/apply-sql-files.mjs supabase/migrations/20260801103000_optimize_sa_summary_rpc_single_pass.sql

CREATE INDEX IF NOT EXISTS idx_sre_sa_code_created_at_desc
ON public.service_reception_entries (sa_employee_code, created_at DESC, id DESC)
WHERE btrim(coalesce(sa_employee_code, '')) <> '';
...
COMMENT ON FUNCTION public.service_reception_entry_in_summary_scope(text, text, text) IS
  'Scope gate for SA summary RPC (non-admin paths). Admin bypass is handled by the caller.';
...
BEGIN
  ...
  WITH gate AS (
    ...
    FROM (SELECT public.is_admin() AS is_admin) g
  ),
  floor_types AS (
    SELECT ARRAY[
...
) IS 'P1-06 Batch F: SA summary counts — single-pass aggregation; advisor/admin fast paths; MATERIALIZED CTEs.';
