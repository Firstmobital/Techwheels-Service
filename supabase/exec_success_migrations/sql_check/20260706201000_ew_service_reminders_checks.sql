...
-- supabase/migrations/20260706201000_ew_service_reminders.sql
...
FROM pg_constraint
WHERE conrelid = 'public.ew_service_reminders'::regclass
ORDER BY conname;
...
WHERE schemaname = 'public'
  AND tablename = 'ew_service_reminders'
ORDER BY indexname;
...
WHERE table_schema = 'public'
  ...
  AND column_name IN (
    'ew_service_reminder_enabled',
    'ew_service_reminder_template_id',
    'ew_service_reminder_template_lang',
    'ew_service_reminder_variable_map',
    'ew_service_reminder_send_time'
  )
...
WHERE n.nspname = 'public'
  AND p.proname IN ('invoke_ew_service_reminder_daily', 'reschedule_ew_service_reminder_cron', 'trg_reschedule_ew_service_reminder_cron');
...
WHERE tgrelid = 'public.wa_agent_config'::regclass
  AND tgname = 'trg_wa_agent_config_reschedule_ew_service';
...
FROM cron.job
WHERE jobname = 'ew-service-reminder-daily-ist';

-- 9) Assert expected schedule/command signature for the job, if registered (13:00 IST = 07:30 UTC).
SELECT COUNT(*) AS matching_job_rows
FROM cron.job
WHERE jobname = 'ew-service-reminder-daily-ist'
  AND schedule = '30 7 * * *'
  AND command ILIKE '%public.invoke_ew_service_reminder_daily()%'
  AND active = true;

-- 10) Row-count sanity check.
SELECT COUNT(*) AS ew_service_reminders_row_count FROM public.ew_service_reminders;
