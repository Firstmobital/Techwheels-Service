...
source_match as (
  select
    ...
    count(*) filter (where j.sr_type ilike '%Service%' and j.closed_date_time is not null) as service_source_rows_with_closed_date_time,
    count(*) filter (where j.sr_type ilike '%Service%' and j.closed_date_time is null) as service_source_rows_with_null_closed_date_time,
    max(j.closed_date_time) filter (where j.sr_type ilike '%Service%') as max_service_source_closed_date_time,
...
select
  ...
  sm.service_source_rows_with_closed_date_time,
  sm.service_source_rows_with_null_closed_date_time,
  sm.max_service_source_closed_date_time,
...
select
  ...
  count(*) filter (where closed_date_time is not null) as source_winners_missing_in_target_with_non_null_closed_date_time,
  count(*) filter (where closed_date_time is null) as source_winners_missing_in_target_with_null_closed_date_time
from target_match
