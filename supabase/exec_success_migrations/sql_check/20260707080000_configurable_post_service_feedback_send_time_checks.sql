...
-- supabase/migrations/20260707080000_configurable_post_service_feedback_send_time.sql
...
WHERE table_schema = 'public'
  ...
  AND column_name = 'post_service_feedback_send_time';
...
WHERE n.nspname = 'public'
  AND p.proname = 'reschedule_post_service_feedback_cron';
...
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public'
  AND p.proname = 'trg_reschedule_post_service_feedback_cron';

-- 4) Trigger is registered on wa_agent_config, fires on UPDATE OF post_service_feedback_send_time.
SELECT
  tgname,
  tgenabled,
...
WHERE tgrelid = 'public.wa_agent_config'::regclass
  AND tgname = 'trg_wa_agent_config_reschedule_psf';
...
SELECT
  ...
  c.command,
  c.active,
  w.post_service_feedback_send_time
FROM cron.job c
CROSS JOIN (
  SELECT post_service_feedback_send_time FROM public.wa_agent_config WHERE id = 1
) w
WHERE c.jobname = 'post-service-feedback-daily-ist';

-- 7) Manual re-run of the reschedule function is idempotent (does not raise, leaves exactly one job row).
SELECT public.reschedule_post_service_feedback_cron(
  (SELECT post_service_feedback_send_time FROM public.wa_agent_config WHERE id = 1)
);
