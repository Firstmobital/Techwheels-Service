...
-- Final alignment fix:
-- If Survey tab is visible (role resolved via get_my_bodyshop_employee_scope),
-- surveyor options must be visible for the same effective dealer scope.
...
BEGIN
  ...
  RETURN v_target_in_scope AND (v_has_module_access OR v_has_bodyshop_role);
END;
$$;
...
CREATE POLICY settings_bodyshop_surveyors_select_v8 ON public.settings_bodyshop_surveyors
...
BEGIN
  ...
  RETURN QUERY
  WITH effective_dealers AS (
    SELECT DISTINCT UPPER(BTRIM(COALESCE(dc.code, ''))) AS dealer_code
    FROM unnest(public.my_effective_dealer_codes()) AS dc(code)
    WHERE BTRIM(COALESCE(dc.code, '')) <> ''
    UNION
    SELECT UPPER(BTRIM(COALESCE(public.my_dealer_code(), '')))
    WHERE BTRIM(COALESCE(public.my_dealer_code(), '')) <> ''
  )
  SELECT
    s.id,
    s.surveyor_name,
  ...
  WHERE UPPER(BTRIM(COALESCE(s.dealer_code, ''))) IN (
    SELECT dealer_code FROM effective_dealers
  )
  ORDER BY s.surveyor_name ASC;
END;
$$;
