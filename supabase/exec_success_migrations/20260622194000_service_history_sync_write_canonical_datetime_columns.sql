-- Purpose:
-- Update realtime Service-History sync flow to write canonical typed date/time columns
-- in public.all_service_data, while keeping legacy text compatibility fields.
--
...
BEGIN
  ...
  ranked AS (
    SELECT
      ...
      public.parse_service_history_datetime_ist(su.service_date_time) AS parsed_service_at
  ...
  SET
    ...
    -- Canonical typed timestamp column (authoritative for long-term logic)
    last_service_at = COALESCE(c.parsed_service_at, t.last_service_at),

    -- Legacy compatibility text column (derived from canonical parsed timestamp)
