...
11. Safeguard: shared `table.tbl` must remain auto-layout (no `table-layout: fixed`) unless a page uses an explicit per-page table class for fixed-column behavior.
12. Table drift guard: default tables must be content-driven (`table-layout: auto`) with one consistent cell gutter (same left/right padding for all th/td); do not enforce per-column fixed widths unless explicitly documented for that page's design artifact.
13. Service Advisor specific guard: keep JC Number, Estimate, Invoice, and Action columns on dynamic content widths with uniform cell spacing; if readability tuning is needed, adjust control internals before column-width forcing.
...
| T-044 | Governance | Enforce cross-page visual synchronization lock (no design-system drift) | home.jsx + service-advisor.jsx + styles.css + components.css | docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md, src/App.css, src/pages/* | IN_PROGRESS | Vinod | 2026-06-02 | 2026-06-07 | - | End-phase release gate now actively executing for AutoDoc production route (`https://techwheels-service.vercel.app/autodoc`): perform visual sync lock audit + fix pass (container/wrapper parity, inline-style elimination, shared class grammar, table rhythm/alignment/spacing) after redesign parity closure. **Pages validated for sync lock**: T-029 (Floor Incharge), T-030 (Technician), T-031 (Warranty Overview tabs), T-032 (Import). **Service Advisor anti-drift policy locked**: content-driven table widths + fixed uniform th/td gutter, no shared `.tbl` fixed layout, no ad-hoc per-column width forcing unless artifact-justified. **Current lock target**: AutoDoc (`src/pages/AutoDocPage.tsx`) - pass 1 complete (32 inline style usages -> 0 in AutoDocPage; converted static visual styling to shared classes in `src/App.css`; build passes). **Next lock targets**: T-014/T-015 (Reception), T-005/T-006 (TopNav/Shell). |
| T-046 | Hotfix | Resolve cross-page table text overlap regression (Service Advisor + Floor Incharge) | Regression audit from shared table CSS | src/App.css, src/pages/ServiceAdvisorPage.tsx, docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md | DONE | Vinod | 2026-06-07 | 2026-06-07 | 2026-06-07 | Incident source commit identified from git blame/log: `27379849ef7d4575e637d9bae9644b3a2d7309fd` by Vinod Bijarnia at `2026-06-07 09:39:33 +0530`, where `.tbl` was changed to `table-layout: fixed;` in `src/App.css`. This forced constrained column sizing with nowrap cell content, causing visible overlap in dense tables. Final resolution: (1) keep shared `.tbl` on auto layout, (2) apply Service Advisor-specific class for content-driven widths, (3) enforce uniform th/td spacing (fixed gutter), and (4) avoid per-column hard widths unless a page-level artifact explicitly requires it. |
...
| T-033 | Remaining Modules | Apply shell + design-system parity to AutoDoc | IMPLEMENTATION_PLAN.md section 7.9 + AutoDoc.html + autodoc.jsx | src/pages/AutoDocPage.tsx | DONE | Vinod | 2026-06-06 | 2026-06-07 | 2026-06-07 | AutoDoc redesign parity completed and stabilized in production flow: Dashboard/Job Card/Damage/Estimate/Submit structures aligned to reference hierarchy; duplicate left-side Job Card documents block removed (single Intake media panel retained), lookup row updated with inline Registration + KM Reading before Fetch enablement, and Job queue table alignment normalized (Estimate/Action left alignment per latest UX directive). Build validation passing (`npm run -s build`). Transitioned to T-044 for final AutoDoc visual sync lock audit/fix. |
| T-045 | Remaining Modules | Apply shell + design-system parity to JobCard page | IMPLEMENTATION_PLAN.md section 7.9 | src/pages/JobCardPage.tsx | PENDING | Vinod | - | 2026-06-07 | - | Split from prior combined T-033 scope to keep AutoDoc closure and JobCard follow-up independently trackable. Execute after AutoDoc T-044 visual sync lock sign-off. |
...
|---|---|---|---|---|
| 2026-06-07 | AutoDoc Visual Sync Lock pass 1 executed in production code: removed all static inline style props from AutoDoc page, replaced them with shared class-based styling, normalized dense table/action/submit-gate styling through reusable classes, and converted KPI tone inline styles to explicit stage tone classes. | T-044 | IN_PROGRESS | `src/pages/AutoDocPage.tsx` (inline styles reduced to zero), `src/App.css` (new `ad-*` utility classes for AutoDoc lock), build validation `npm run -s build` ✓ (723 modules). |
| 2026-06-07 | AutoDoc redesign parity closure recorded and tracker execution shifted to final Visual Sync Lock for production AutoDoc route. Scope updates: (1) marked T-033 DONE for `src/pages/AutoDocPage.tsx`; (2) moved T-044 to IN_PROGRESS with AutoDoc as active lock target; (3) split JobCard follow-up into T-045 PENDING to avoid mixed completion state. Current execution focus is anti-drift audit/fix on `https://techwheels-service.vercel.app/autodoc` (wrapper/container parity, shared class enforcement, spacing/alignment/token consistency). | T-033, T-044, T-045 | IN_PROGRESS | docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md (section 9 task updates + this log entry); recent implementation evidence in `src/pages/AutoDocPage.tsx`; build validation already passing via `npm run -s build`. |
| 2026-06-04 | Second-stage alert integrity fix applied after cross-report linkage review: `Review >3d`, `SOP pending >2d`, and `Approved-not-settled >5d` were still row-inflated because interrelated source tables can emit duplicate rows for the same claim. Implemented claim-level dedupe in Critical Alerts using a stable claim key (`jobCardNumber` primary, posting/fallback secondary) before count/rendering and debug total computation. | T-031, T-043, TR-034 | COMPLETE | `src/pages/reports/warranty/WarrantyOverviewReport.tsx` (`getWorkflowClaimKey`, `dedupeWorkflowClaims`, deduped alert predicates + debug counts). Validation scripts: `/tmp/warranty_alerts_dedupe_audit.js` and post-patch dump simulation showed significant reductions; build passed (`npm run build --silent`). |
| 2026-06-04 | Broadened workflow-linkage hardening beyond Alert 1: all status-driven calculations in Warranty Overview now operate on workflow-eligible rows (non-blank workflow status and non-Claim-Settlement category where stage semantics apply). Updated areas include pipeline stage counts, rejection distribution, TAT stage buckets, alert predicate diagnostics/debug buckets, and status-based Financial/Operations counters (Pending WC, AMC pending, payment-pending KPI stage filter). | T-031, T-043, TR-034 | COMPLETE | `src/pages/reports/warranty/WarrantyOverviewReport.tsx` (`workflowStatusRecords` adoption across status metrics); build validation: `npm run build --silent` ✓ (723 modules, no TS errors). |
| 2026-06-04 | Critical Alerts inflation fix implemented in Warranty Overview: alert computation now uses workflow-eligible records only (excludes `Claim Settlement` category and blank-status rows) before applying Created/Review/SOP/Approved/Rejection predicates. This removes settlement snapshot rows from `Created >24h` and aligns alerts with real workflow-state claims. Build validation passed. | T-031, T-043, TR-034 | COMPLETE | Code change: `src/pages/reports/warranty/WarrantyOverviewReport.tsx` (`isWorkflowAlertEligible`, `workflowAlertRecords`, updated `computedAlerts` + debug alert counting). Build: `npm run build --silent` ✓ (723 modules, no TS errors). |
| 2026-06-04 | Root-cause diagnosis for inflated `Created >24h` alert completed from authoritative dump. Of the 3263-row Created cohort reproduced by diagnostic pass, 3177 rows come from `warranty_claim_settlement_report_data` where workflow status fields are absent/blank; these rows still carry posting/invoice fields (`posting_date_yyyy_mm_dd`, `invc_date_yyyy_mm_dd`, `posting_document_number`) and are therefore settlement-stage data, not true created-stage claims. Remaining cohort is primarily explicit `claim_status=Created` records (86) plus small residual rows. | T-031, T-043 | FINDING CONFIRMED | Diagnostic scripts run: `/tmp/created_alert_diagnose.js` and `/tmp/created_blank_status_profile.js` against `local_folder/backups/chunks/full_database.sql.part_*`; code path reference: `normalizeStatusBucket` + status extraction in `src/pages/reports/warranty/WarrantyOverviewReport.tsx`. |
| 2026-06-04 | Pre-validation run for Warranty Overview Critical Alerts against authoritative dump completed before redesign-forward work. Live route `https://techwheels-service.vercel.app/reports/warranty/warranty-overview` is currently auth-gated (sign-in wall), so direct UI count capture is blocked in this pass. Added and executed `scripts/warranty-critical-alerts-validate-dump.js` to mirror current `WarrantyOverviewReport.tsx` logic (`extractStatusValue`, `normalizeStatusBucket`, age-date parsing, and 5 alert predicates) and recompute counts from dump chunks. | T-031, T-043 | PARTIAL (DB validation complete, live read blocked by auth gate) | Script output @ 2026-06-04T05:28:57.731Z from authoritative dump chunks: allRecords = Created>24h 3264, Review>3d 2401, SOP>2d 2469, Approved-not-settled>5d 91, Rejection-blank 3, Total 8228; Sitapura PV (3000840-like) = 1787/1367/1399/62/0 (Total 4615); Sitapura EV (500A840-like) = 1197/921/951/4/2 (Total 3075); Sitapura combined = 2984/2288/2350/66/2 (Total 7690). Evidence files: `scripts/warranty-critical-alerts-validate-dump.js`, `src/pages/reports/warranty/WarrantyOverviewReport.tsx`, `local_folder/backups/chunks/full_database.sql.part_003`. |
| 2026-06-03 | DB-truth wiring audit completed for Warranty route against authoritative dump + reference artifacts: confirmed warranty schema contract remains 7 JSONB source tables with branch/location/portal checks and updated_at triggers; confirmed role enum in users table is admin/manager/staff/viewer only (no super_admin literal); confirmed warranty runtime currently registers one route (`warranty-overview`) while W-001 TR-024..TR-040 remain pending/partial for 15 additional report views. | T-031, T-043, T-044 | AUDIT COMPLETE | Authoritative dump mirror: local_folder/backups/chunks/full_database.sql.part_000 (users role check/modules), part_002 (user_module_permissions/users COPY), part_003 (warranty COPY + triggers); implementation: src/pages/reports/warranty/index.ts + src/pages/reports/warranty/WarrantyOverviewReport.tsx; plan cross-check: docs/Implementation_plans/webversion/categories/warranty/active/WARRANTY-001_WARRANTY_REPORT_IMPORT_AND_REPORTING_PLAN.md TR-024..TR-040 |
...
| 2026-06-03 | Warranty overview parity correction completed against reference warranty.jsx/Warranty Reports.html: rebuilt Critical Alerts with 5 left-border KPI cards + warning note + 5 alert table cards; rebuilt Financial with 5 left-border KPI cards + invoice pending table + 20% revenue blocks/product/month matrix + AMC/SPL tables; rebuilt Operations with pending WC/Updation cards, PDI root cause, top parts PV/EV, back order, recovery opportunity; compile/build revalidated | T-031 | COMPLETE | src/pages/reports/warranty/WarrantyOverviewReport.tsx, docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md, npm run build ✓ (723 modules, 0 TS errors) |
| 2026-06-02 | Visual drift governance update completed: locked cross-page synchronization rules to prevent web redesign divergence (Home baseline mandated across all modules), and recorded Service Advisor parity alignment to shared pagehead/card/tbl/summary system | T-028, T-044 | COMPLETE | docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md (sections 2.2, 5.4, 7.2A, 7.6, 9, 11 updated), src/pages/ServiceAdvisorPage.tsx, src/App.css, npm run build ✓ built in 858ms |
...
| 2026-06-02 | Settings redesign handoff resumed; tracker updated to enforce single active task before implementation edits | T-021 | IN_PROGRESS | docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md (T-021 status/owner/start/notes updated) |
...
git diff docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md

# Verify section numbering is correct
grep -n "^## [0-9]" docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md

# Verify no duplicate section numbers
grep -n "^## [0-9]" docs/Implementation_plans/webversion/categories/redesign/active/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md | cut -d: -f2 | sort | uniq -d
