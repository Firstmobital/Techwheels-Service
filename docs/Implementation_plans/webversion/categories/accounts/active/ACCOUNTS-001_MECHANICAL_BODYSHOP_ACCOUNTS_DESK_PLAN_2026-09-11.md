...
- **Busy Export** (Mechanical only) downloads `accounts-mechanical-busy-payments.xlsx`: Invoice date, voucher_no, Account DR, Account CR, Amount DR, Amount CR, Reference no. Cash/UPI/card receipt lines only. Account DR: `CASH AT SITAPURA` / `PAYTM WALLET` / `CREDIT CARD A/C`. Account CR is BUSY Party Name (same lookup as Excel `account_name`). cheque/bank/other are skipped with a warning. Pending cases without receipts are omitted. Column **Invoice date** is the payment voucher date: `payment_received_date` → Accounts `invoice_date` → unique DMS labour `invoice_date`. `posted_at` and Mark Done are not used. Voucher eligibility (`invoice_date >= 2026-09-02`) is separate and unchanged.
