...
5. Invoice Voucher rows: always emit `SPARE PARTS @18%` and `LABOUR CHARGES @18%` (including Amount 0). Emit `SPARE PARTS @5%` only when matched Parts data contains a genuine 5% GST line. Put Round Off in `Rounded Off (-)` or `Rounded Off (+)` on the Labour row only. Do not emit a Round Off item row. Do not use `eligible × 3` as a row-count rule.
