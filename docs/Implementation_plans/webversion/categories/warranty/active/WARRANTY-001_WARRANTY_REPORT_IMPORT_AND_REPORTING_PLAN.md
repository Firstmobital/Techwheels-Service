...
| TR-018 | Exact formula parity from source sheets | Explicit per-source metric formula map | docs/Implementation_plans/webversion/categories/warranty/active/WARRANTY-001_WARRANTY_REPORT_IMPORT_AND_REPORTING_PLAN.md (planned), future code files | Pending | Signed formula sheet + unit checks |
...
| **TR-038-CORRECTED** | **Define JSONB extraction mappings per source type (REPLACES schema migration)** | **Extraction contract documentation** | **docs/Implementation_plans/webversion/categories/warranty/evidence/WARRANTY-001_JSONB_EXTRACTION_MAPPINGS.md** | **Pending** | **Audit source files + document field->JSONB path mappings** |
...
> Promotion note (2026-06-29): TR-043-NEW (row above) is Done and Verified; promoted to category reference summary per `docs/STRUCTURE_GUIDE.md` Section 28/30 — see `docs/web/modules/warranty/README.md` ("Promoted Implementation Items"). Row kept here, not removed, because TR-044-NEW below depends on it and the broader plan remains active.

| **TR-044-NEW** | **Super-admin equivalent full-scope contract for 28-report wiring** | **`admin@firstmobital.com` must see all mapped dealer-code data across all fuel types in every warranty report** | **docs/Implementation_plans/webversion/categories/warranty/active/WARRANTY-001_WARRANTY_REPORT_IMPORT_AND_REPORTING_PLAN.md; src/pages/reports/warranty/*** | **Pending** | **For `admin@firstmobital.com` with mapped dealer codes `3000840`, `500A840`, `3001440`: totals/rows must match unfiltered dataset across all 28 reports** |
