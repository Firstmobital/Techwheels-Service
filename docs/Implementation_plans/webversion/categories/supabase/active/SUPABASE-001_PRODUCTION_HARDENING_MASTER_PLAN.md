...
| P1-05 | Critical | Remove fetch-all patterns from reports and warranty JSON extractors | Team | In Progress | 2026-06-08 |  | Batch A shared-module rollout extended: remaining `.range` offset loops in `src/lib/reportQueries.ts` and `mobile/src/lib/reportQueries.ts` were removed from targeted report fetch paths and replaced with bounded single-pass queries (`limit(MAX_REPORT_FETCH_ROWS)`) while preserving existing function contracts. Build checks passed (`npm run build`; `npm --prefix ./mobile run -s typecheck`). | 2026-06-26 | Re-run post-deploy interval recapture under comparable traffic and verify sustained delta improvement on tracked queryids |
| P1-06 | High | Replace OFFSET scans with cursor pagination in large tables/views | Team | In Progress | 2026-06-25 |  | Shared report module offset-pattern reduction completed for targeted paths in `src/lib/reportQueries.ts` and `mobile/src/lib/reportQueries.ts`; page-level keyset rollouts remain active (`src/pages/SATrackerPage.tsx`, `src/pages/TechnicianPage.tsx`). | 2026-06-26 | Validate production impact in next comparable-traffic recapture; if residual regressions persist, continue conversion in remaining non-targeted modules (`partsReportQueries`) |
