...
Last updated: 2026-08-07 (snapshot 14.48 + SA save 57014 fix implemented; deploy pending)
...
- Authoritative dump audit source: `supabase/backups/full_metadata.sql` (schema-only, ~1.77 MB), manifest at `supabase/evidence/authoritative_metadata_manifest.json`
- Authoritative dump timestamp: 2026-07-28 04:58:52 UTC (sha256=3dd4899f5f28d5ce69673c3283d6e9970f7b4a2f35023db9809a3a2c4d60a72d); pg_dump --schema-only --no-owner; aligned with snapshot 14.46 audit (2026-07-28 10:29 IST)
- Latest automated audit: snapshot **14.46** (`2026-07-28__04-59-02-873Z`); guard **blocked_requires_checklist**; reception list queries still #2/#3 by total_ms; Batch E partial win (page-2/page-1 call ratio ~3.8→~2.0) but slim summary scan `3827816949739656130` adds ~728k ms at ~4384ms mean
...
| P1-06 | High | Replace OFFSET scans with cursor pagination in large tables/views | Team | In Progress | 2026-06-25 |  | **Batch E deployed; verified snapshot 14.46 (2026-07-28).** Page-2/page-1 call ratio improved ~3.8→~2.0 (14.43–14.45 vs 14.45–14.46) but reception queries still #2/#3 (`852176900607336119` 4408 calls/1721ms mean; `3787216458397661678` 7681/604ms). New cost: slim summary scan `3827816949739656130` (166 calls, ~4384ms mean). **Batch F (next):** replace background slim scan with DB RPC/scoped counts; eliminate remaining fetch-all (global search, floor, technician, `-3550207178760076775`). Evidence: [P1_06_RECEPTION_LIST_BOUNDING](../evidence/P1_06_RECEPTION_LIST_BOUNDING_2026-07-22.md), [Batch E](../evidence/P1_06_RECEPTION_PAGINATED_LIST_BATCH_E_2026-07-24.md), [Batch F plan](../evidence/P1_06_RECEPTION_SUMMARY_RPC_BATCH_F_2026-07-28.md). | 2026-07-28 | Implement Batch F summary RPC; paginate/remove global search fetch-all; post-deploy audit vs 14.46 |
...
| P1-11 | Critical | Log-driven performance tracker (rolling updates from every new capture) | Team | In Progress | 2026-06-25 |  | Rolling evidence synced to snapshot **14.46** (`2026-07-28__04-59-02-873Z`); Batch E partial verify documented. Guard blocked (+15.6M ms vs 14.45). | 2026-07-28 | Sync tracker after Batch F deploy; target guard improvement vs 14.46 |
...
| P1-13 | Critical | Fix service_reception_entries 57014 (all hot paths) | Team | In Progress | 2026-08-07 | 2026-08-08 | **Systemic RLS timeout** on direct PostgREST SELECT/INSERT/UPDATE — not user-specific. **3 migrations (apply in order):** `20260807120000` (SA save + sync trigger, applied), `20260807130000` (reception list/create/update), `20260807140000` (read by id/reg, JC lookup, invoice done, bulk import, reg search, portal on create/update). **Frontend:** `reception.ts` (all hot paths), ReceptionPage, PartsRequirement, mobile reception. | 2026-08-07 | Apply 07130000 + 07140000; deploy frontend; smoke-test Reception + SA |
...
| P2-03 | Critical | Reconcile deployed schema with migration history | Team | In Progress | 2026-06-05 |  | Metadata dump refreshed **2026-07-28 04:58:52 UTC** via `npm run db:backup:metadata`; sha256=3dd4899f5f28d5ce69673c3283d6e9970f7b4a2f35023db9809a3a2c4d60a72d, size=1.769 MB. Includes migrations through `20260727140000_business_roles_csv_helpers.sql` (verify manifest). | 2026-07-28 | Reconcile pending repo migrations vs `cron.job` / deployed state after Batch F |
...
| P2-05 | High | Resolve schema drift where app queries depend on objects not present in authoritative dump | Team | In Progress | 2026-06-05 |  | Snapshot **14.46** postgres error: `service_parts_stock_snapshot_data.on_hand_qty does not exist`. Authoritative dump column is **`on_hand_quantity`**. App bug: `src/pages/PartsSPMDashboardPage.tsx` selects `on_hand_qty`. Legacy: `vw_parts_latest_stock` / `vw_parts_consumption_trend` still unresolved. | 2026-07-28 | Fix `on_hand_qty` → `on_hand_quantity` in PartsSPMDashboardPage; verify no other callers |
...
## 4A) Current Priorities (post snapshot 14.46)

Ordered work for the next deploy cycle:

| # | Task ID | Action | Why |
|---|---------|--------|-----|
| 1 | P1-13 | **Deploy SA save 57014 fix** — apply `20260807120000`, ship frontend RPC call | PATCH 500 + 170× statement timeout on SA Save (14.48 evidence) |
| 2 | P1-06 Batch F | Replace SA `fetchReceptionSummaryFieldsByDateRange` with summary-count RPC or scoped head-count queries | Slim scan `3827816949739656130` ~4384ms/call — negates Batch E gains |
| 2 | P1-06 | Bound/paginate global search, floor incharge, technician fallback fetch-all | `-3550207178760076775` and related queryids still climbing |
| 3 | P2-05 | Fix `on_hand_qty` → `on_hand_quantity` in `PartsSPMDashboardPage.tsx` | 14.46 postgres error; column exists in metadata dump |
| 4 | P1-08 | Realtime subscription inventory + scope reduction (Batch C Phase 2) | Still #1 by total_ms (+273k calls vs 14.45) |
| 5 | P1-11 | Post-deploy audit vs **14.46** after above ship | Verify `3827816949739656130` gone; reception delta_calls flatten |

**DB truth for all schema work:** `supabase/backups/full_metadata.sql` (manifest `2026-07-28T04:58:52Z`).
...
| 2026-08-07 (automated audit cycle) | - | - | - | - | - | - | - | - | Top query -2876120296317350531 calls=1654782 total_ms=14017464.73 mean_ms=8.47; comparison=regressed; delta_total_ms_sum=30153611.19 |
| 2026-08-12 (automated audit cycle) | - | - | - | - | - | - | - | - | Top query -2876120296317350531 calls=2082724 total_ms=17008499.35 mean_ms=8.17; comparison=regressed; delta_total_ms_sum=15938307.98 |
...
| 2026-08-07 | Copilot | Automated Supabase audit cycle appended run summary (2026-08-07 12:27:54 IST) and refreshed plan evidence block from generated audit artifacts. |
...
| 2026-08-12 | Copilot | Automated Supabase audit cycle appended run summary (2026-08-12 10:05:52 IST) and refreshed plan evidence block from generated audit artifacts. |
...
| P0 | Phase F1: Query-shape stabilization | F1.1 Reception list/count hardening | P1-06 | Step 2: Complete keyset pagination rollout for `(created_at,id)` list paths. | In Progress | Batch E deployed; 14.46 partial verify. **Batch F next:** summary RPC + remaining fetch-all elimination |
...
### 14.48 Capture Snapshot: 2026-08-07 (Automated Audit Cycle)

What was captured:
- Timestamp (IST): 2026-08-07 12:27:54 IST
- Capture mode: automated_supabase_audit_cycle
- Top queryid: -2876120296317350531 (calls=1654782, total_ms=14017464.73, mean_ms=8.47)
- Platform logs capture status: auth=ok, edge_functions=ok, realtime=ok, storage=ok, database_health=ok
- Comparison vs previous run (2026-07-28__05-28-38-056Z): status=regressed, delta_total_ms_sum=30153611.19, delta_calls_sum=534072
- Top regressions by delta_total_ms: -2876120296317350531 (5295202.39); 852176900607336119 (4467168.14); -1491976781120316096 (2219340.4)
- Top postgres log messages (by frequency): canceling statement due to statement timeout (170); function gen_random_bytes(integer) does not exist (105); there is no unique or exclusion constraint matching the ON CONFLICT specificatio (37)

Compact Top 10 (run-local):
| rank | queryid | calls | total_ms | mean_ms |
|---:|---|---:|---:|---:|
| 1 | -2876120296317350531 | 1654782 | 14017464.73 | 8.47 |
| 2 | 852176900607336119 | 7791 | 12092549.45 | 1552.12 |
| 3 | 3787216458397661678 | 8951 | 4894882.24 | 546.85 |
| 4 | 8843009277484467611 | 932 | 4361492.43 | 4679.71 |
| 5 | -397576279058981298 | 1190 | 4058020.02 | 3410.10 |
| 6 | 8976932172498995662 | 14968 | 2894287.41 | 193.37 |
| 7 | -2147031708195470770 | 2296 | 2624890.64 | 1143.25 |
| 8 | -3550207178760076775 | 6947 | 2522195.65 | 363.06 |
| 9 | 3109077696112254485 | 893 | 2502647.39 | 2802.52 |
| 10 | -1851842182524549347 | 32201 | 2488035.67 | 77.27 |
...
- **P1-13 fix shipped (2026-08-07):** SA Save 57014 — direct UPDATE replaced by `service_advisor_save_reception_entry` RPC; sync trigger hardened. Post-deploy verification pending.
...
Next action:
- **P1-13:** Apply migration `20260807120000_fix_sa_save_reception_entry_timeout.sql`, deploy frontend, re-run post-deploy audit — target reduction in `57014` count and PATCH 500 on `service_reception_entries`.
- Re-run the cycle after the next production traffic window and validate that comparison status moves toward improved.

### 14.49 Capture Snapshot: 2026-08-12 (Automated Audit Cycle)

What was captured:
- Timestamp (IST): 2026-08-12 10:05:52 IST
- Capture mode: automated_supabase_audit_cycle
- Top queryid: -2876120296317350531 (calls=2082724, total_ms=17008499.35, mean_ms=8.17)
- Platform logs capture status: auth=ok, edge_functions=ok, realtime=ok, storage=ok, database_health=ok
- Comparison vs previous run (2026-08-07__06-57-54-789Z): status=regressed, delta_total_ms_sum=15938307.98, delta_calls_sum=439380
- Top regressions by delta_total_ms: 4868736656759022764 (5949732.15); -2876120296317350531 (2991034.62); 9034250094703622185 (2452766.49)
- Top postgres log messages (by frequency): canceling statement due to statement timeout (652); column all_service_data.registration_no does not exist (71); function gen_random_bytes(integer) does not exist (66)

Compact Top 10 (run-local):
| rank | queryid | calls | total_ms | mean_ms |
|---:|---|---:|---:|---:|
| 1 | -2876120296317350531 | 2082724 | 17008499.35 | 8.17 |
| 2 | 852176900607336119 | 7797 | 12094748.46 | 1551.21 |
| 3 | 4868736656759022764 | 6773 | 5949732.15 | 878.45 |
| 4 | 8843009277484467611 | 1074 | 5145534.15 | 4791.00 |
| 5 | 3787216458397661678 | 8951 | 4894882.24 | 546.85 |
| 6 | -397576279058981298 | 1356 | 4756405.52 | 3507.67 |
| 7 | -2147031708195470770 | 2890 | 3418940.21 | 1183.02 |
| 8 | 8976932172498995662 | 17876 | 3133257.95 | 175.28 |
| 9 | 3109077696112254485 | 1008 | 2969380.37 | 2945.81 |
| 10 | 1012486468402746359 | 387 | 2646509.52 | 6838.53 |
