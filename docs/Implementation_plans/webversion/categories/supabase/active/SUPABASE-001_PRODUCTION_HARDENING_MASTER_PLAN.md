...
- Runtime (latest confirmed dashboard/runtime evidence, 2026-06-26 10:54 IST): RAM 65%, CPU 15%, disk 34%, connections 24/60, Disk IO 45%; latest automated audit snapshot 14.31 remains regressed, with the same query-shape pressure still concentrated on `service_reception_entries` and related list/count paths
...
- Advisory state: Security Advisor Errors already cleared (0 on 2026-06-08 milestone); current operational risk remains the Disk IO budget depletion warning plus query-shape regression pressure
...
- Maintenance note: no fresh dashboard snapshot was supplied in this update, so the tracker remains anchored to the latest recorded 2026-06-26 checkpoint and 14.31 audit cycle
...
| P1-05 | Critical | Remove fetch-all patterns from reports and warranty JSON extractors | Team | In Progress | 2026-06-08 |  | Batch A shared-module rollout remains code-verified and in production-facing review; latest snapshot 14.31 stayed regressed, so performance validation is still pending. | 2026-06-29 | Re-run post-deploy interval recapture under comparable traffic and verify sustained delta improvement on tracked queryids |
| P1-06 | High | Replace OFFSET scans with cursor pagination in large tables/views | Team | In Progress | 2026-06-25 |  | Shared report module offset-pattern reduction is complete for targeted paths; latest snapshot 14.31 remained regressed, so the remaining keyset rollouts still need production validation. | 2026-06-29 | Validate production impact in next comparable-traffic recapture; if residual regressions persist, continue conversion in remaining non-targeted modules (`partsReportQueries`) |
...
| P1-08 | High | Reduce Realtime WAL polling cost and fan-out | Team | In Progress | 2026-06-25 |  | `realtime.list_changes` remains high-frequency (`queryid=-2876120296317350531`, `calls=612039`, `total_ms=3832695.82`, `mean_ms=6.26`); latest audit evidence still points to subscription fan-out as a major cost driver, and no new teardown validation was supplied in this maintenance pass. | 2026-06-29 | Complete per-screen subscription inventory and enforce unsubscribe teardown checks before next capture window |
| P1-09 | High | Eliminate expensive exact-count list patterns in PostgREST paths | Team | In Progress | 2026-06-25 |  | Exact-count mitigation remains active on audited hotspots (`src/pages/reports/master-data/MasterDataNullCountsReport.tsx`, `mobile/src/app/(tabs)/autodoc.old.tsx`); latest snapshot 14.31 remained regressed, so exact-count/list cleanup must continue. | 2026-06-29 | Continue exact-count elimination on shared high-traffic list endpoints and keep heavy counts explicit/opt-in only |
| P1-10 | Critical | Disk IO budget incident response (query-shape mitigation first) | Team | In Progress | 2026-06-25 | 2026-06-27 | Batch A mitigation + recapture executed; latest run `2026-06-26__12-38-13-502Z` (snapshot `14.31`) remains regressed (`delta_total_ms_sum=2381491.74`, `delta_calls_sum=14517`) with strongest regressions on `-5344960703026327435`, `4251000708073776526`, `3220864789079889211`. No fresh dashboard/runtime values were supplied in this maintenance pass, so incident response stays anchored to the latest manual checkpoint and snapshot 14.31 regression. | 2026-06-29 | Run next interval recapture under comparable traffic after broader shared-module rollout (P1-05/P1-06/P1-09) and compare same queryid set |
| P1-11 | Critical | Log-driven performance tracker (rolling updates from every new capture) | Team | In Progress | 2026-06-25 |  | Rolling evidence model remains active with latest-two snapshots + compact top-10 table. Latest snapshot advanced to `14.31`; tracker synchronization remains aligned to that evidence and the latest manual checkpoint, with no additional capture supplied in this maintenance update. | 2026-06-29 | For each new capture: rank by `delta_total_ms`, sync P1-05..P1-10 evidence, and enforce retention bounds in Sections 5/6/14 |
...
| 2026-06-29 (tracker maintenance pass) | 65% | 15% | 34% | 24/60 | - | - | - | - | Baseline preserved from the latest confirmed dashboard checkpoint and 14.31 audit cycle; no fresh runtime values were supplied in this maintenance update |
...
| 2026-06-26 | Copilot | Finished Batch A shared-module offset/range elimination pass for targeted paths in web/mobile `reportQueries`; replaced residual `.range` loops with bounded `limit(MAX_REPORT_FETCH_ROWS)` fetches and re-validated build/typecheck. |
...
| 2026-06-26 | Copilot | Automated Supabase audit cycle appended run summary (2026-06-26 18:08:13 IST) and refreshed plan evidence block from generated audit artifacts. |
...
| 2026-06-29 | Copilot | Maintained the long-term Supabase tracker by refreshing the baseline snapshot, updating impacted P1 tracker rows to reflect the latest 14.31 evidence, and appending one metrics and one change-log row without inventing new runtime values. |
...
Cycle anchor:
- Latest snapshot: `14.31` (2026-06-26 18:08:13 IST)
- Current movement: `regressed` (`delta_total_ms_sum=2381491.74`)
...
- Not yet verified (performance): latest recapture snapshot `14.31` remains `regressed`; production interval verification still required after this code rollout.
...
| P0 | Phase F3: Delta verification gate | F3.1 Controlled interval recapture | P1-10 | Step 6: Run post-deploy interval recapture in next real traffic window. | Done | Verified (snapshot 14.31 captured) |
| P0 | Phase F3: Delta verification gate | F3.2 Evidence synchronization | P1-11 | Step 7: Re-rank regressions by `delta_total_ms` and sync tracker rows P1-05..P1-10. | Done | Verified (rows synced to 14.31 evidence) |
