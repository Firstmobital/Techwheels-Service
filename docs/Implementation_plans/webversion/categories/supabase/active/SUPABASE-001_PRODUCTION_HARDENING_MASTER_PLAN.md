...
| P1-13 | Critical | Fix SA job-card save statement timeout (57014) | Team | In Progress | 2026-08-07 | 2026-08-08 | **SA save (applied):** `20260807120000` + frontend RPC. **Reception page (code complete):** `20260807130000` adds `list_reception_entries_page`, `create_reception_entry`, `update_reception_entry` SECURITY DEFINER RPCs; ReceptionPage load probes removed; `reception.ts` list/create/update switched to RPCs. Root cause: direct PostgREST on `service_reception_entries` under expensive RLS — not a user/account issue. | 2026-08-07 | Apply `20260807130000`; deploy frontend; smoke-test Reception + SA Save |
