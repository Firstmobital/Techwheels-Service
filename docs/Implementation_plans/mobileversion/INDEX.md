# Mobile Version Plans Index

Last Updated: 2026-08-11
Scope: Mobile app implementation plans only
Authority: This index is the routing source for mobileversion plans

---

## 1) Folder Contract

All mobile planning documents must be inside:
- `docs/Implementation_plans/mobileversion/categories/<category>/{active|evidence|inactive}/`

Mobile categories currently in use:
- `auth`
- `autodoc`
- `core-shell`
- `help-tickets`
- `import`
- `operations`
- `program`
- `reports`
- `redesign`

---

## 2) Main Tracker

Use this file as daily command center:
- `docs/Implementation_plans/mobileversion/IMPLEMENTATION_TRACKER.md`

---

## 3) Active Plan Registry

| Plan ID | Category | Status | Owner | File |
|---|---|---|---|---|
| MOBILE-000 | program | Active | Product + Engineering + Copilot | `docs/Implementation_plans/mobileversion/categories/program/active/MOBILE-000_OVERVIEW.md` |
| MOBILE-001 | program | Active | Product + Engineering + Copilot | `docs/Implementation_plans/mobileversion/categories/program/active/MOBILE-001_EXPO_IMPLEMENTATION_PLAN.md` |
| MOBILE-005 | autodoc | In Progress | Product + Mobile | `docs/Implementation_plans/mobileversion/categories/autodoc/active/MOBILE-005_AUTODOC_GPS_STAMP_PARITY_PLAN.md` |
| MOBILE-006 | autodoc | Pending | Product + Mobile | `docs/Implementation_plans/mobileversion/categories/autodoc/active/MOBILE-006_GOOGLE_SATELLITE_HYBRID_STAMP_PLAN.md` |
| MOBILE-007 | core-shell | In Progress | Mobile Team | `docs/Implementation_plans/mobileversion/categories/core-shell/active/MOBILE-007_PLATFORM_HOME_SUPERAPP_TRACKER.md` |
| MOBILE-HELP-001 | help-tickets | In Progress | Mobile Team + Platform | `docs/Implementation_plans/mobileversion/categories/help-tickets/active/MOBILE-HELP-001_EMPLOYEE_HELP_LITE_PLAN.md` |
| MOBILE-008 | operations | Pending | Mobile Team | `docs/Implementation_plans/mobileversion/categories/operations/active/MOBILE-008_BACKGROUND_PROCESSING_STATE_UX_PLAN.md` |
| MOBILE-009 | redesign | Active | Mobile Team | `docs/Implementation_plans/mobileversion/categories/redesign/active/MOBILE-009_MOBILE_APP_REDESIGN_PARITY_TRACKER.md` |
| MOBILE-010 | program | Active | Mobile Team | `docs/Implementation_plans/mobileversion/categories/program/active/MOBILE-010_MOBILE_PROGRAM_MASTER_TRACKER.md` |
| MOBILE-BP-RD | redesign | In Progress | Mobile Team | `docs/Implementation_plans/mobileversion/categories/redesign/active/redesign_bodypaint_module.md` |

---

## 4) Evidence Registry

| Category | File |
|---|---|
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/MOBILE-002_EXECUTION_CHECKLIST.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/MOBILE-003_ARCHITECTURE.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/MOBILE-004_FEATURE_MAPPING.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/MOBILE-QUICK_REFERENCE.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/REFERENCE_PROJECT_INTEGRATION.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/PHASE-006_OFFLINE_SUPPORT.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/PHASE-006_TESTING_GUIDE.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/PHASE-006_COMPLETION_SUMMARY.md` |
| program | `docs/Implementation_plans/mobileversion/categories/program/evidence/PHASE-006_METRO_TROUBLESHOOTING.md` |
| redesign | `docs/Implementation_plans/mobileversion/categories/redesign/evidence/SCREEN_REDESIGN_PARITY_AUDIT_TEMPLATE.md` |

---

## 5) Inactive Registry

No files registered yet in the new `mobileversion` structure.

---

## 6) Archive Link

Completed mobile plans live at:
- `docs/Implementation_plans/completed/mobileversion/`
