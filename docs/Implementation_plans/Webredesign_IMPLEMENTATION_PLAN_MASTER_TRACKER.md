...
| T-044 | Governance | Enforce cross-page visual synchronization lock (no design-system drift) | home.jsx + service-advisor.jsx + styles.css + components.css | docs/Implementation_plans/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md, src/App.css, src/pages/* | IN_PROGRESS | Vinod | 2026-06-02 | 2026-06-07 | - | End-phase release gate now actively executing for AutoDoc production route (`https://techwheels-service.vercel.app/autodoc`): perform visual sync lock audit + fix pass (container/wrapper parity, inline-style elimination, shared class grammar, table rhythm/alignment/spacing) after redesign parity closure. **Pages validated for sync lock**: T-029 (Floor Incharge), T-030 (Technician), T-031 (Warranty Overview tabs), T-032 (Import). **Current lock target**: AutoDoc (`src/pages/AutoDocPage.tsx`) - pass 1 complete (32 inline style usages -> 0 in AutoDocPage; converted static visual styling to shared classes in `src/App.css`; build passes). **Next lock targets**: T-028 (Service Advisor), T-014/T-015 (Reception), T-005/T-006 (TopNav/Shell). |
| T-046 | Hotfix | Resolve cross-page table text overlap regression (Service Advisor + Floor Incharge) | Regression audit from shared table CSS | src/App.css, docs/Implementation_plans/Webredesign_IMPLEMENTATION_PLAN_MASTER_TRACKER.md | DONE | Vinod | 2026-06-07 | 2026-06-07 | 2026-06-07 | Incident source commit identified from git blame/log: `27379849ef7d4575e637d9bae9644b3a2d7309fd` by Vinod Bijarnia at `2026-06-07 09:39:33 +0530`, where `.tbl` was changed to `table-layout: fixed;` in `src/App.css`. This forced constrained column sizing with nowrap cell content, causing visible overlap in dense tables. Hotfix applied: reverted shared `.tbl` rule to auto layout by removing `table-layout: fixed;` so columns can size to content and overflow uses existing horizontal table wrapper (`.tbl-wrap`). |
