...
| T-033 | Remaining Modules | Apply shell + design-system parity to AutoDoc and JobCard | IMPLEMENTATION_PLAN.md section 7.9 | src/pages/AutoDocPage.tsx, src/pages/JobCardPage.tsx | IN_PROGRESS | Vinod | 2026-06-06 | 2026-06-07 | - | **RESUMED AFTER SESSION CRASH**: AutoDoc parity work continued using local reference `local_folder/Reference/WebVersionRedesignReference/AutoDoc.html` + `autodoc.jsx`. Implemented reference-style Dashboard Job queue table, Job Card right-side Intake media card, existing-job Damage segmented stage workflow (Pre/Under/Post), and Submit tab two-column Submission gates + Claim actions. Latest pass aligned Job Card intake-first UX: initial view now shows single Registration + Fetch bar and expands to full details only after lookup state change, matching reference flow intent. Build validation passing (`npm run -s build`). Remaining scope: JobCardPage shell/design-system parity and final T-044 sync-lock sweep. |
