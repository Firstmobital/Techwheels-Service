...
- Payroll domain: `payroll_compensation`, `payroll_attendance`, `payroll_advances`, `payroll_advance_schedules`, `payroll_months`, `payroll_entries`, `payroll_adjustments`, `payroll_employee_incentives` (percentage or fixed amount lines; snapshot `payroll_entries.incentive_amount`), `payroll_security_settings` (hash only; not client-selectable), `payroll_security_grants`, `payroll_security_attempt_state`. Month lock/unlock is RPC-only (`payroll_finalize_month`, `payroll_unlock_month`). Non-admin payroll writes require `payroll_has_security_grant()`.
