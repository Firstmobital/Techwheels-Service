...
- Payroll Processing **Earned Base Total** card Export downloads a 13-column bank payout XLSX (`Bank Payout` sheet, header row). Scope is the same Month / Department / Branch / Salary Type set as the card; Search does not apply. Amount is persisted `net_payable` (not `earned_base`). Rows with `net_payable === 0` are omitted; `SALARY1…` IDs are assigned after that filter. The card KPI remains earned-base and is not required to match the file total. Company Account is `39169685030`. SBI → DCR, else NEFT. Missing bank fields on eligible (non-zero) rows stay as blank cells; zero-net employees are not exported and do not block the file. Distinct from the finalized toolbar net-payable export. No schema change.
