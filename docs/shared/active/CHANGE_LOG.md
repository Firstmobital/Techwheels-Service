...
- Payroll Processing **Earned Base Total** card has an Export control that downloads a 13-column bank payout XLSX (`Bank Payout` sheet, header row). Amount is each scoped row's persisted `earned_base` — the same Month / Department / Branch / Salary Type set as the card. Search does not change the file. Company Account is `39169685030` (not the technician earnings account). SBI detection matches the existing earnings helper (bank name / IFSC `SBIN` → DCR, else NEFT). Missing bank fields stay on the file as blank cells so the Amount total still matches the card. Filename follows `Bank_BaseSalary_Sept2026.xlsx`. Distinct from the finalized toolbar net-payable export. No schema change.
