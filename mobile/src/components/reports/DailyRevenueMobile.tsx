...
export default function DailyRevenueMobile({ branch, dateFilter }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const data = await getDailyRevenueReport(branch, dateFilter)
        if (!active) return
        setRows(data)
      } catch (err: any) {
        if (!active) return
        setError(err?.message || 'Failed to load daily revenue report')
      } finally {
        if (!active) return
        setLoading(false)
      }
  ...
  const exportCsv = async () => {
    try {
      const exportRows = rows.map((row) => ({
        ...
        avg_billing_per_vehicle: row.avgBillingPerVehicle,
      }))

      await shareCsv(toCsv(exportRows), 'daily-revenue-report')
    } catch (err: any) {
      Alert.alert('Export Failed', err?.message || 'Could not export daily revenue CSV')
    }
  }
    ...
    return (
      <View className="bg-white border border-slate-200 rounded-xl p-4">
        <View className="items-center py-6">
          ...
          <Text className="text-slate-500 mt-2">Loading daily revenue report...</Text>
        </View>
      </View>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <View className="bg-white border border-slate-200 rounded-xl p-4">
        <Text className="text-slate-700 font-semibold">Daily Revenue Report</Text>
      ...
      <View className="bg-white border border-slate-200 rounded-xl p-4">
        <View className="flex-row items-start justify-between">
          <View className="flex-1 pr-3">
            <Text className="text-base font-semibold text-slate-900">Daily Revenue Report</Text>
            <Text className="text-xs text-slate-500 mt-1">Daily revenue breakdown by date from PSF Revenue Report data.</Text>
          </View>
        ...
        <View className="flex-row flex-wrap mt-3">
          ...
          <View className="w-1/2 pl-2 pb-2"><View className="rounded-lg bg-pink-50 border border-pink-100 p-3"><Text className="text-[10px] uppercase text-pink-600 font-semibold">Total Invoices</Text><Text className="text-lg font-bold text-pink-900 mt-1">{totals.totalInvoices.toLocaleString('en-IN')}</Text></View></View>
          <View className="w-full pb-2"><View className="rounded-lg bg-indigo-50 border border-indigo-100 p-3"><Text className="text-[10px] uppercase text-indigo-600 font-semibold">Days Reported</Text><Text className="text-lg font-bold text-indigo-900 mt-1">{totals.days.toLocaleString('en-IN')}</Text></View></View>
        </View>
      </View>

      <View className="bg-white border border-slate-200 rounded-xl p-4">
        <Text className="text-sm font-semibold text-slate-900 mb-2">Daily Revenue Table</Text>
        ...
        <View className="flex-row flex-wrap mb-2">
          {([
            ...
            ['totalRevenue', 'Total'],
            ['vasRevenue', 'VAS'],
            ['avgBillingPerVehicle', 'Avg/Vehicle'],
          ...
          <Text className="text-[11px] text-slate-500 self-center">Direction: {sortDirection === 'asc' ? 'Ascending' : 'Descending'}</Text>
        </View>

        <ScrollView horizontal showsHorizontalScrollIndicator>
          <View>
            <View className="flex-row bg-slate-100 rounded-t-md">
              <Text className="w-24 px-2 py-2 text-[11px] font-semibold text-slate-700">Date</Text>
              <Text className="w-20 px-2 py-2 text-[11px] font-semibold text-slate-700 text-right">Vehicles</Text>
              <Text className="w-20 px-2 py-2 text-[11px] font-semibold text-slate-700 text-right">Invoices</Text>
              ...
              <Text className="w-28 px-2 py-2 text-[11px] font-semibold text-slate-700 text-right">Avg/Vehicle</Text>
            </View>

            {sortedRows.map((row) => (
              <View key={row.date} className="flex-row border-b border-slate-100">
                <Text className="w-24 px-2 py-2 text-xs text-slate-800">{row.date}</Text>
                <Text className="w-20 px-2 py-2 text-xs text-slate-700 text-right">{row.vehicleCount.toLocaleString('en-IN')}</Text>
                <Text className="w-20 px-2 py-2 text-xs text-slate-700 text-right">{row.invoiceCount.toLocaleString('en-IN')}</Text>
                ...
                <Text className="w-28 px-2 py-2 text-xs text-slate-700 text-right">Rs. {formatCurrency(row.avgBillingPerVehicle)}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
