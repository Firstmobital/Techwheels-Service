...
import type { ComplaintPortalResponse } from './types'
 ...
 */
export async function getComplaintByToken(token: string): Promise<ComplaintPortalResponse> {
  const { data, error } = await supabase.rpc('get_complaint_by_token', { p_token: token })
  if (error) throw error
  return data as ComplaintPortalResponse
}
...
): Promise<ComplaintPortalResponse> {
 ...
 */
export async function addCustomerMessage(token: string, body: string): Promise<ComplaintPortalResponse> {
 ...
 */
export async function reopenComplaint(token: string, reason: string): Promise<ComplaintPortalResponse> {
 ...
 */
export async function acknowledge(complaintId: bigint): Promise<{ status: string }> {
 ...
 */
export async function startProgress(complaintId: bigint): Promise<{ status: string }> {
 ...
 * Resolve a complaint
 */
export async function resolve(complaintId: bigint): Promise<{ status: string }> {
 ...
 * Close a complaint
 */
export async function close(complaintId: bigint): Promise<{ status: string }> {
...
): Promise<{ link_id: bigint; token: string; url: string }> {
