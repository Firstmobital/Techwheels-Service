...
export default function SalarySlipReportTab({ payrollMonth, monthInput, onMonthChange }: Props) {
  ...
  function exportConsolidated() {
    const headers = [
      ...
      'Earned Base', 'SA Variable', 'Tech Variable', 'Bodyshop Variable', 'Incentive', 'Additions', 'Gross',
    ...
    const rows = visibleEntries.map((e) => {
      ...
      return [
        ...
        e.earned_base, e.sa_variable_earning, e.technician_variable_earning, e.bodyshop_variable_earning ?? 0, e.incentive_amount ?? 0, e.custom_additions, e.gross_payout,
        ...
        <div id="salary-slip" style={{ maxWidth: '520px', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '1rem' }}>
          ...
          <table style={{ width: '100%', fontSize: '0.78rem', borderCollapse: 'collapse' }}>
            <tbody>
              ...
              {Number(selectedEntry.incentive_amount ?? 0) > 0 && (
                <tr><td>Incentive</td><td style={{ textAlign: 'right' }}>{formatCurrency(Number(selectedEntry.incentive_amount ?? 0))}</td></tr>
              )}
