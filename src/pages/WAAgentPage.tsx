...
export default function WAAgentPage() {
  ...
  const [campaignForm, setCampaignForm] = useState({
    ...
    flow_type: 'blast', template_id: null as number | null,
  })
  ...
  async function sendManualMessage() {
    ...
    try {
      // Message already saved to DB above
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: '10px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                  ...
                  {messages.map(msg => {
                    ...
                    return (
                      <div key={msg.id} style={{ display: 'flex', justifyContent: isOut ? 'flex-end' : 'flex-start' }}>
                        <div style={{ maxWidth: '70%', background: isOut ? '#dcfce7' : '#fff', borderRadius: isOut ? '12px 12px 4px 12px' : '12px 12px 12px 4px', padding: '0.55rem 0.8rem', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}>
                ...
                <div style={{ padding: '0.65rem 0.85rem', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '0.5rem', flexShrink: 0, background: '#fff' }}>
                  <textarea value={manualMsg} onChange={e => setManualMsg(e.target.value)}
                    ...
                    placeholder="Type a manual reply (overrides AI)… Enter to send" className="inp" rows={2}
                    style={{ flex: 1, resize: 'none', fontSize: '0.82rem' }} />
                  <button className="btn btn--sm" style={{ background: '#25D366', color: '#fff', border: 'none', fontWeight: 700, alignSelf: 'flex-end', padding: '0.5rem 0.9rem' }}
                    onClick={sendManualMessage} disabled={sendingManual || !manualMsg.trim()}>
                    {sendingManual ? '⏳' : '💬 Send'}
                  </button>
                </div>
              </div>
                      ...
                      {['draft','rejected'].includes(tpl.status) && (
                        <button className="btn btn--ghost btn--sm" style={{ fontSize:'0.72rem' }}
                          onClick={() => {
                            ...
                            setTemplateForm({ display_name: tpl.display_name, name: tpl.name, category: tpl.category, language: tpl.language, header_type: tpl.header_type || 'NONE', header_text: tpl.header_text || '', body_text: tpl.body_text, footer_text: tpl.footer_text || '', buttons: JSON.stringify(tpl.buttons || [], null, 2), variable_examples: JSON.stringify(tpl.variable_examples || [], null, 2), campaign_type: tpl.campaign_type || 'service_reminder' })
                            setShowTemplateForm(true)
                          }}>✏️ Edit</button>
                      )}
                      {['draft','rejected'].includes(tpl.status) && (
                        <button className="btn btn--primary btn--sm" style={{ fontSize:'0.72rem' }} disabled={submittingTemplate === tpl.id}
                          onClick={async () => {
              ...
              <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'flex-start', justifyContent:'center', overflowY:'auto', padding:'1.5rem' }}>
                <div style={{ background:'#fff', borderRadius:'12px', padding:'1.5rem', width:'100%', maxWidth:'680px', boxShadow:'0 20px 60px rgba(0,0,0,0.3)', marginTop:'1rem' }}>
                  ...
                  <label className="field" style={{ marginBottom:'0.75rem' }}>
                    ...
                    <div style={{ fontSize:'0.65rem', color:'#94a3b8' }}>Use {'{{1}}'} {'{{2}}'} {'{{3}}'} etc. for variables. Keep under 1024 chars.</div>
                  </label>
                  <label className="field" style={{ marginBottom:'0.75rem' }}>
                  ...
                  <label className="field" style={{ marginBottom:'0.75rem' }}>
                    ...
                    <div style={{ fontSize:'0.65rem', color:'#94a3b8' }}>Types: QUICK_REPLY · URL (needs url field) · PHONE_NUMBER (needs phone field). Max 3 buttons.</div>
                  </label>
                  <label className="field" style={{ marginBottom:'1rem' }}>
                    ...
                    <textarea className="inp" rows={4} value={templateForm.variable_examples}
                      ...
                      style={{ fontFamily:'monospace', fontSize:'0.75rem', resize:'vertical' }} />
                    <div style={{ fontSize:'0.65rem', color:'#94a3b8' }}>Provide realistic example values for each {'{{variable}}'}. Meta uses these to review your template.</div>
                  </label>
                  <div style={{ display:'flex', gap:'0.5rem', justifyContent:'flex-end' }}>
                    <button className="btn btn--ghost" onClick={() => { setShowTemplateForm(false); setEditingTemplate(null) }}>Cancel</button>
                    <button className="btn btn--primary" disabled={!templateForm.body_text || !templateForm.name || !templateForm.display_name}
                      onClick={async () => {
                        ...
                        const payload = { display_name: templateForm.display_name, name: templateForm.name, category: templateForm.category, language: templateForm.language, status: 'draft', header_type: templateForm.header_type === 'NONE' ? null : templateForm.header_type, header_text: templateForm.header_text || null, body_text: templateForm.body_text, footer_text: templateForm.footer_text || null, buttons: parsedButtons, variable_examples: parsedExamples, campaign_type: templateForm.campaign_type, updated_at: new Date().toISOString() }
                        if (editingTemplate) {
        ...
        {tab === 'followups' && (
          <div>
            ...
            <div style={{ background: 'linear-gradient(135deg,#eff6ff 0%,#dbeafe 100%)', border: '1px solid #bfdbfe', borderRadius: '10px', padding: '0.85rem 1rem', marginBottom: '1rem', fontSize: '0.8rem', color: '#1e40af' }}>
              <strong>How it works:</strong> When you run a campaign, click <em>Enroll in Follow-up</em> — the AI automatically sends Day 1 → Day 3 → Day 7 reminders to customers who haven't booked yet.
            </div>
                ...
                <div key={step.id} style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '0.75rem', marginBottom: '0.65rem', background: step.is_active ? '#fff' : '#f8fafc' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.4rem' }}>
                      <button className="btn btn--ghost btn--sm" style={{ fontSize: '0.72rem' }} onClick={() => { setEditingStep(step.id); setStepDraft(step.message_template) }}>✏️ Edit</button>
                      <button className="btn btn--ghost btn--sm" style={{ fontSize: '0.72rem', color: step.is_active ? '#ef4444' : '#16a34a' }}
                        onClick={async () => { await supabase.from('wa_followup_steps').update({ is_active: !step.is_active }).eq('id', step.id); await loadAll() }}>
                  ...
                  {editingStep === step.id ? (
                    <div>
                      <textarea className="inp" rows={4} style={{ fontSize: '0.78rem', fontFamily: 'monospace', width: '100%', resize: 'vertical' }} value={stepDraft} onChange={e => setStepDraft(e.target.value)} />
                      <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.4rem' }}>
                        <button className="btn btn--primary btn--sm" style={{ fontSize: '0.75rem' }} disabled={savingStep}
                          onClick={async () => { setSavingStep(true); await supabase.from('wa_followup_steps').update({ message_template: stepDraft }).eq('id', step.id); setEditingStep(null); setSavingStep(false); await loadAll() }}>
                    ...
                    <div style={{ fontSize: '0.77rem', color: '#374151', whiteSpace: 'pre-wrap', background: '#f8fafc', padding: '0.5rem 0.65rem', borderRadius: '6px', lineHeight: 1.5 }}>{step.message_template}</div>
                  )}
                </div>
              ))}
            </Section>
            <Section title="🚀 Enroll Campaign in Follow-up Sequence">
              {campaigns.filter(c => ['Completed','Running'].includes(c.status)).length === 0 ? (
                <p style={{ fontSize: '0.8rem', color: '#94a3b8' }}>No completed campaigns yet.</p>
              ) : (
                  ...
                  {campaigns.filter(c => ['Completed','Running'].includes(c.status)).map(camp => (
                    <div key={camp.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0.75rem', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.72rem', color: '#64748b' }}>{camp.sent_count || 0} sent · {camp.booked_count || 0} booked · {camp.status}</div>
                      </div>
                      <button className="btn btn--primary btn--sm" style={{ fontSize: '0.75rem', whiteSpace: 'nowrap' }} disabled={enrollingCamp === camp.id}
                        onClick={async () => {
                          setEnrollingCamp(camp.id)
                          const { data } = await supabase.auth.getSession()
                          const res = await fetch('https://jmdndcphkmaljhwgzqxq.supabase.co/functions/v1/wa-followup-cron', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${data.session?.access_token}` }, body: JSON.stringify({ enroll_campaign_id: camp.id, sequence_id: 1 }) })
                          const result = await res.json()
                          setEnrollingCamp(null)
                          setToast(result.error ? `❌ ${result.error}` : `✅ Enrolled ${result.enrolled || 0} contacts!`)
                          await loadAll()
                        }}>
        ...
        {tab === 'settings' && config && (
          <div style={{ maxWidth: '760px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <button className="btn btn--primary btn--sm" style={{ marginLeft: 'auto' }} onClick={saveConfig} disabled={saving}>{saving ? 'Saving…' : '💾 Save Settings'}</button>
            </div>
            <Section title="📱 Meta WhatsApp API">
              <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', padding: '0.75rem 0.9rem', marginBottom: '0.85rem', fontSize: '0.78rem', color: '#1e40af' }}>
                ...
                <div style={{ background: '#fff', border: '1px solid #93c5fd', borderRadius: '5px', padding: '0.3rem 0.55rem', fontFamily: 'monospace', fontSize: '0.75rem', marginBottom: '0.35rem', wordBreak: 'break-all', color: '#1e3a8a' }}>https://jmdndcphkmaljhwgzqxq.supabase.co/functions/v1/wa-webhook</div>
                <div style={{ marginBottom: '0.25rem' }}>3. Set Verify Token to the value below</div>
                <div>4. Subscribe to: <strong>messages</strong></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
                <label className="field"><span className="label">Phone Number ID</span><input className="inp" placeholder="From Meta → API Setup" value={config.meta_phone_number_id || ''} onChange={e => setConfig(p => p ? { ...p, meta_phone_number_id: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Permanent Access Token</span><input className="inp" type="password" placeholder="EAA…" value={config.meta_access_token || ''} onChange={e => setConfig(p => p ? { ...p, meta_access_token: e.target.value } : p)} /></label>
                <label className="field" style={{ gridColumn: 'span 2' }}>
                  <span className="label">Webhook Verify Token</span>
                  <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                    ...
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => { const token = Math.random().toString(36).slice(2) + '_tw_' + Date.now().toString(36); setConfig(p => p ? { ...p, wa_verify_token: token } : p) }}>🔀 Generate</button>
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => { navigator.clipboard.writeText(config.wa_verify_token || ''); showToast('✅ Copied!') }}>📋 Copy</button>
                  </div>
                </label>
                <label className="field">
                  ...
                  <div style={{ fontSize: '0.68rem', color: '#94a3b8' }}>If set, campaign booking starts with native WhatsApp Flow form.</div>
                </label>
                <label className="field">
            ...
            <Section title="🧠 OpenAI API (AI Replies)">
              <label className="field"><span className="label">OpenAI API Key</span><input className="inp" type="password" placeholder="sk-proj-…" value={config.openai_api_key || ''} onChange={e => setConfig(p => p ? { ...p, openai_api_key: e.target.value } : p)} /></label>
            </Section>
            <Section title="🤖 Agent Identity">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
                <label className="field"><span className="label">Agent Name</span><input className="inp" placeholder="e.g. Riya" value={config.agent_name} onChange={e => setConfig(p => p ? { ...p, agent_name: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Business Name</span><input className="inp" placeholder="e.g. Techwheels Service" value={config.business_name} onChange={e => setConfig(p => p ? { ...p, business_name: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Working Hours</span><input className="inp" placeholder="Mon-Sat 9AM-6PM" value={config.working_hours} onChange={e => setConfig(p => p ? { ...p, working_hours: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Max AI Turns per Conversation</span><input className="inp" type="number" min={3} max={25} value={config.max_ai_turns} onChange={e => setConfig(p => p ? { ...p, max_ai_turns: parseInt(e.target.value) || 10 } : p)} /></label>
                <label className="field" style={{ gridColumn: 'span 2' }}><span className="label">Available Branches (comma separated)</span><input className="inp" value={(config.available_branches || []).join(', ')} onChange={e => setConfig(p => p ? { ...p, available_branches: e.target.value.split(',').map(s => s.trim()).filter(Boolean) } : p)} /></label>
              </div>
            </Section>
            ...
            <Section title="📨 Message Templates">
              <label className="field"><span className="label">Greeting Message</span><textarea className="inp" rows={3} value={config.greeting_message} onChange={e => setConfig(p => p ? { ...p, greeting_message: e.target.value } : p)} style={{ resize: 'vertical' }} /></label>
              <label className="field" style={{ marginTop: '0.65rem' }}><span className="label">Booking Confirmation Message</span><textarea className="inp" rows={5} value={config.booking_confirm_msg} onChange={e => setConfig(p => p ? { ...p, booking_confirm_msg: e.target.value } : p)} style={{ resize: 'vertical' }} /></label>
              <label className="field" style={{ marginTop: '0.65rem' }}><span className="label">Closing / Thank You Message</span><textarea className="inp" rows={2} value={config.closing_message} onChange={e => setConfig(p => p ? { ...p, closing_message: e.target.value } : p)} style={{ resize: 'vertical' }} /></label>
            </Section>
            <Section title="🚨 Escalation & Slot Settings">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <label className="field"><span className="label">Daily Slot Capacity</span><input className="inp" type="number" min={1} max={200} value={config.daily_slot_capacity ?? 40} onChange={e => setConfig(p => p ? { ...p, daily_slot_capacity: Number(e.target.value) } : p)} /></label>
                <label className="field"><span className="label">SA WhatsApp Number</span><input className="inp" placeholder="919876543210" value={config.sa_whatsapp_number ?? ''} onChange={e => setConfig(p => p ? { ...p, sa_whatsapp_number: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Escalation Email</span><input className="inp" type="email" value={config.escalation_email ?? ''} onChange={e => setConfig(p => p ? { ...p, escalation_email: e.target.value } : p)} /></label>
                <label className="field"><span className="label">Escalation Phone</span><input className="inp" value={config.escalation_phone ?? ''} onChange={e => setConfig(p => p ? { ...p, escalation_phone: e.target.value } : p)} /></label>
              </div>
              <label className="field" style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                <input type="checkbox" checked={config.staff_notify_on_escalation ?? true} onChange={e => setConfig(p => p ? { ...p, staff_notify_on_escalation: e.target.checked } : p)} />
        ...
        {tab === 'test' && (
          <div style={{ maxWidth: '600px' }}>
            ...
            <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '8px', padding: '0.65rem 0.85rem', marginBottom: '1rem', fontSize: '0.78rem', color: '#92400e' }}>
              ⚡ This uses the real AI + DB — enter any test phone number. Set your OpenAI key in Settings first.
            </div>
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '0.85rem', marginBottom: '0.85rem', display: 'flex', gap: '0.65rem', alignItems: 'flex-end' }}>
              ...
              <button className="btn btn--ghost btn--sm" onClick={resetTestConv} disabled={!testConvId}>🔄 Reset</button>
            </div>
            <div style={{ background: '#e5ddd5', borderRadius: '10px', overflow: 'hidden', border: '1px solid #d1d5db', marginBottom: '0.75rem' }}>
              ...
              <div style={{ height: '350px', overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.6rem', background: '#e5ddd5' }}>
                {testChat.length === 0 && <div style={{ textAlign: 'center', color: '#666', fontSize: '0.78rem', padding: '2rem' }}>Start typing to test the AI agent conversation flow</div>}
                {testChat.map((m, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '78%', background: m.role === 'user' ? '#dcf8c6' : '#fff', borderRadius: m.role === 'user' ? '10px 10px 2px 10px' : '10px 10px 10px 2px', padding: '0.5rem 0.75rem', boxShadow: '0 1px 2px rgba(0,0,0,0.12)' }}>
              ...
              <div style={{ background: '#f0f0f0', padding: '0.55rem 0.75rem', display: 'flex', gap: '0.5rem', alignItems: 'flex-end' }}>
                <textarea value={testMessage} onChange={e => setTestMessage(e.target.value)}
                  ...
                  placeholder="Type a message as customer… (Enter to send)" className="inp" rows={2}
                  style={{ flex: 1, resize: 'none', fontSize: '0.85rem', borderRadius: '20px', padding: '0.45rem 0.85rem' }} />
                <button onClick={sendTestMessage} disabled={testLoading || !testMessage.trim()}
                  style={{ width: '40px', height: '40px', borderRadius: '50%', background: testLoading || !testMessage.trim() ? '#94a3b8' : '#25D366', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>➤</button>
              </div>
            </div>
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '0.75rem' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#475569', marginBottom: '0.5rem' }}>💡 Quick test messages:</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                {['Hi','Yes, I want to book a service','Next Monday works for me','Morning slot please, Sitapura branch','YES, confirm the booking'].map(msg => (
                  <button key={msg} onClick={() => setTestMessage(msg)}
                    style={{ padding: '0.2rem 0.55rem', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '0.72rem', color: '#475569', cursor: 'pointer' }}>{msg}</button>
                ))}
              </div>
            </div>
          </div>
