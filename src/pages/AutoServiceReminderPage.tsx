...
interface ReminderLog {
  ...
  assumed_next_service_date: string
  reminder_type: '20_day' | '9_day' | '3_day'
...
export default function AutoServiceReminderPage() {
  ...
  async function saveConfig() {
    ...
    try {
      const { error: e } = await supabase
        .from('wa_agent_config')
        .update({
          auto_reminder_enabled:       configDraft.auto_reminder_enabled,
  ...
  async function runJob() {
    ...
    setRunResult(null)
    try {
      const { data, error: e } = await supabase.functions.invoke('wa-auto-service-reminder', {
        body: { dry_run: dryRun },
      })
      if (e) throw e
  ...
  const stats = useMemo<Stats>(() => {
    ...
    return {
      ...
      sent_20_day:  todayLogs.filter(l => l.reminder_type === '20_day').length,
      sent_9_day:   todayLogs.filter(l => l.reminder_type === '9_day').length,
  ...
  const selectedTemplate = templates.find(t => t.id === configDraft?.auto_reminder_template_id)
  const varExamples = selectedTemplate?.variable_examples || []

  // ── Render ───────────────────────────────────────────────────────────────
  if (loading) {
    return (
      <div className="p-8 text-center text-gray-500">Loading Auto Service Reminder data…</div>
    )
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Auto Service Reminder</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Sends WhatsApp reminders 20, 9, and 3 days before a customer's assumed service due date.
          </p>
        </div>
        <button
          onClick={fetchAll}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          ...
          <StatCard label="20-Day Reminders"      value={stats.sent_20_day} color="text-purple-700" />
          <StatCard label="9-Day Reminders"       value={stats.sent_9_day}  color="text-indigo-700" />
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={configDraft.auto_reminder_enabled}
                onChange={e => setConfigDraft(d => d ? { ...d, auto_reminder_enabled: e.target.checked } : d)}
                className="w-4 h-4 accent-green-600"
              />
              <span className="text-sm font-medium text-gray-700">Enable daily auto reminders</span>
            </label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                value={configDraft.auto_reminder_template_id ?? ''}
                onChange={e => setConfigDraft(d => d ? { ...d, auto_reminder_template_id: e.target.value ? Number(e.target.value) : null } : d)}
              >
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm"
                value={configDraft.auto_reminder_template_lang}
                onChange={e => setConfigDraft(d => d ? { ...d, auto_reminder_template_lang: e.target.value } : d)}
                ...
                {varExamples.map(ex => (
                  <div key={ex.name} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={configDraft.auto_reminder_variable_map[ex.name] ?? ''}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            >
              ...
              <option value="20_day">20 Day</option>
              <option value="9_day">9 Day</option>
            ...
            <tbody className="divide-y divide-gray-100">
              {pageRows.length === 0 ? (
                ...
                <tr key={l.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{fmtDate(l.assumed_next_service_date)}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      l.reminder_type === '3_day'  ? 'bg-red-100 text-red-700' :
                      l.reminder_type === '9_day'  ? 'bg-yellow-100 text-yellow-700' :
