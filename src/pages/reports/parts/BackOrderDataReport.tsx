...
// Also includes a "Browse All Sheet 2" mode to view all Co-Dealer stock data.
...
type ViewMode = 'search' | 'browse_sheet2'
...
export default function BackOrderDataReport() {
  ...
  // ── Combined search (Sheet 1 + Sheet 2) ────────────────────────────────────
  const doSearch = useCallback(async (partNo: string) => {
    ...
    try {
      ...
      if (vorRes.error) console.error('[BackOrder] VOR query error:', vorRes.error)
      if (cpRes.error) console.error('[BackOrder] CP query error:', cpRes.error)

      setVorRows(vorRes.data ?? [])
      setCpRows(cpRes.data ?? [])
    } catch (err) {
      console.error('[BackOrder] Search error:', err)
      setVorRows([])
      setCpRows([])
    } finally {
      setLoading(false)
  ...
  // ── Browse all Sheet 2 data ─────────────────────────────────────────────────
  const browseAllSheet2 = useCallback(async () => {
    ...
    try {
      ...
      if (error) console.error('[BackOrder] Browse error:', error)
      setAllCpRows(data ?? [])
    } catch (err) {
      console.error('[BackOrder] Browse error:', err)
      setAllCpRows([])
    } finally {
  ...
  const totalAllCpQty = allCpRows.reduce((sum, r) => sum + (r.available_qty ?? 0), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-gray-200">
        ...
        <button
          type="button"
          onClick={() => { setViewMode('browse_sheet2'); if (allCpRows.length === 0) void browseAllSheet2() }}
          className={`px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${
            viewMode === 'browse_sheet2'
              ? 'border-blue-600 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Browse All Sheet 2 (Co-Dealer)
        </button>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">
              {allCpRows.length > 0
                ? `${allCpRows.length} Co-Dealer stock records loaded.`
                : 'No Sheet 2 data uploaded yet. Go to /import to upload AVL WITH CP STOCK data.'}
            </p>
            <button
              type="button"
              onClick={() => void browseAllSheet2()}
              disabled={browseLoading}
              className="rounded-lg bg-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-300 disabled:opacity-50"
            >
            ...
            <>
              <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
                <p className="text-sm font-semibold text-blue-700">
                  Total Available Qty across all Co-Dealers: {totalAllCpQty}
                </p>
              </div>
              ...
              <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                <div className="border-b border-gray-100 px-4 py-3">
                  <h4 className="text-sm font-bold text-gray-900">
                    AVL WITH CP STOCK <span className="font-normal text-gray-400">— Sheet 2 (All Co-Dealer Data)</span>
                  </h4>
                </div>
