...
interface AgentConfig {
  ew_renewal_enabled: boolean
  ew_renewal_template_id: number | null
  ew_renewal_template_lang: string
  ew_renewal_variable_map: Record<string, string>
  ew_renewal_send_time: string
}
...
export default function EWRenewalReminderPage() {
    ...
    try {
      const [logsRes, cfgRes, tplRes] = await Promise.all([
        ...
        supabase
          ...
          .select('ew_renewal_enabled, ew_renewal_template_id, ew_renewal_template_lang, ew_renewal_variable_map, ew_renewal_send_time')
          .eq('id', 1)
          .single(),
        supabase
          .from('wa_templates')
  ...
  async function saveConfig() {
    ...
    try {
      const { error: e } = await supabase
        .from('wa_agent_config')
        .update({
          ew_renewal_enabled:       configDraft.ew_renewal_enabled,
          ew_renewal_template_id:   configDraft.ew_renewal_template_id,
          ew_renewal_template_lang: configDraft.ew_renewal_template_lang,
          ew_renewal_variable_map:  configDraft.ew_renewal_variable_map,
          ew_renewal_send_time:     configDraft.ew_renewal_send_time,
        })
        .eq('id', 1)
      if (e) throw e
      setConfigSaved(true)
  ...
  async function runJob() {
    ...
    setRunResult(null)
    try {
      const { data, error: e } = await supabase.functions.invoke('wa-ew-renewal-reminder', {
        body: { dry_run: dryRun },
      })
      if (e) throw e
  ...
  const stats = useMemo<Stats>(() => {
    ...
    return {
      ...
      sent_10_day: todayLogs.filter(l => l.reminder_type === '10_day').length,
      sent_3_day:  todayLogs.filter(l => l.reminder_type === '3_day').length,
  ...
  }, [logs, filterDate, filterType, filterStatus])

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE))
  const pageRows   = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE)

  // ── Selected template variable examples ─────────────────────────────────
  const selectedTemplate = templates.find(t => t.id === configDraft?.ew_renewal_template_id)
  const varExamples = selectedTemplate?.variable_examples || []

  // ── Render ───────────────────────────────────────────────────────────────
  if (loading) {
    return (
      <div className="p-8 text-center text-gray-500">Loading EW Renewal Reminder data…</div>
    )
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">EW Renewal Reminder</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Sends a WhatsApp "Renew Now" reminder 10 and 3 days before a customer's Extended Warranty expires.
          </p>
        </div>
        <button
          onClick={fetchAll}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <StatCard label="10-Day Reminders" value={stats.sent_10_day} color="text-indigo-700" />
          <StatCard label="3-Day Reminders"  value={stats.sent_3_day}  color="text-blue-700" />
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={configDraft.ew_renewal_enabled}
                onChange={e => setConfigDraft(d => d ? { ...d, ew_renewal_enabled: e.target.checked } : d)}
                className="w-4 h-4 accent-green-600"
              />
              <span className="text-sm font-medium text-gray-700">Enable daily EW renewal reminders</span>
            </label>
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${configDraft.ew_renewal_enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
              {configDraft.ew_renewal_enabled ? 'Enabled' : 'Disabled'}
            </span>
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={configDraft.ew_renewal_send_time.slice(0, 5)}
              onChange={e => setConfigDraft(d => d ? { ...d, ew_renewal_send_time: e.target.value + ':00' } : d)}
            />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                value={configDraft.ew_renewal_template_id ?? ''}
                onChange={e => setConfigDraft(d => d ? { ...d, ew_renewal_template_id: e.target.value ? Number(e.target.value) : null } : d)}
              >
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm"
                value={configDraft.ew_renewal_template_lang}
                onChange={e => setConfigDraft(d => d ? { ...d, ew_renewal_template_lang: e.target.value } : d)}
                ...
                {varExamples.map(ex => (
                  <div key={ex.name} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={configDraft.ew_renewal_variable_map[ex.name] ?? ''}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        {(filterDate || filterType !== 'all' || filterStatus !== 'all') && (
          <button
            onClick={() => { setFilterDate(''); setFilterType('all'); setFilterStatus('all'); setPage(1) }}
                ...
                <tr key={l.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      l.reminder_type === '3_day' ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'
                    }`}>
