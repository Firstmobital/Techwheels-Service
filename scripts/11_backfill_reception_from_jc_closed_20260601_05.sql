-- Backfill Reception entries from job_card_closed_data for 01-05 Jun 2026 (inclusive)
--
-- IMPORTANT
-- 1) Run this in Supabase SQL Editor manually.
-- 2) Date window is inclusive: 2026-06-01 to 2026-06-05.
...
WITH params AS (
  SELECT
    ...
    'JC Closed Backfill (01-05 Jun 2026)'::text AS backfill_source,
...
source_rows AS (
  ...
  CROSS JOIN params p
  WHERE coalesce(j.closed_date_time::date, j.created_date_time::date, j.created_at::date) BETWEEN p.start_date AND p.end_date
    AND nullif(btrim(j.job_card_number), '') IS NOT NULL
...
-- Post-checks
-- -----------------------------------------------------------------------------
-- SELECT count(*) AS reception_rows_in_window
-- FROM public.service_reception_entries
-- WHERE source = 'JC Closed Backfill (01-05 Jun 2026)';
--
